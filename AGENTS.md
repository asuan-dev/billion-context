...
7. **修复重启、切模型或失败重试时**：`plugin-conversations` 也包含普通缓存键映射，不能据此认定插件身份；保留旧映射的压缩状态，并以原始历史边界约束重算后的可压缩范围，回归入口是 `tests/subagent-namespace.test.ts` 和 `tests/preflight-fail-fast.test.ts`。
